...
(defun my-eww-open-in-new-window (url &optional arg)
  "Open EWW in a new window."
  (interactive (list (read-string "Enter URL or keywords: ")
                     current-prefix-arg))
  (let ((display-buffer-alist
         (list (cons "\\*eww\\*" (cons #'display-buffer-pop-up-window nil)))))
    (eww-browse-url url arg)))

(global-set-key (kbd "C-c w") 'my-eww-open-in-new-window)
